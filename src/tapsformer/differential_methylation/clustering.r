...
print(dim(methylation_levels))  # Check dimensions
print(head(methylation_levels))  # Inspect the first few rows

var_filter <- apply(methylation_levels, 1, var)
methylation_levels_filtered <- methylation_levels[var_filter > 0.01, ]
methylation_levels_scaled <- t(scale(t(methylation_levels_filtered)))

print("tumour data loaded")

# Step 2: Perform hierarchical clustering
# Compute distances and clustering
dist_matrix <- dist(t(methylation_levels_scaled))  # Distance matrix for samples
...
heatmap <- Heatmap(methylation_levels_scaled,
